...
class LabelsBase:
    ...
    def add_label(self, key: str, value: str):
        if key in self.labels.keys():
            ...
            raise InfobloxException(message=f"The label {key} is not part of defined {self.labels}", status=500)
...
class Transform(metaclass=abc.ABCMeta):
    """
    An interface class used by all different modules
    """
    def __init__(self):
        pass
    ...
    def parse(self):
        """
        Use this method to manage the data from source into a format used to
        create metrics
        :return:
        """
